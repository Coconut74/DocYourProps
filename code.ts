...
type DocOptions = {
  ...
  // Repeated sibling groups confirmed for collapse (LLM-refined override of
  // the inline structural detection). Each entry lists all sibling member
  // keys reduced to one (repKey kept). Absent → pure structural detection
  // runs inline in the sandbox.
  repetitionGroups?: { parentKey: string; memberKeys: string[]; repKey: string }[];
  // LLM-chosen pin target per anchor key (Fix B): anchorKey → absolute
  // child-index key whose box best represents the visible element. Absent →
  // the paint-aware resolveLeafTarget heuristic decides.
  anchorTargets?: Record<string, string>;
  // Whether the "Exemple" sheet is requested.
  exemple: boolean;
  // LLM-generated usage scenarios for the "Exemple" sheet. Each entry is a set
  // of prop values (keyed by stripped prop name) applied to a fresh instance.
  // Absent / empty → buildExemplesSection falls back to mechanical examples.
  exemples?: AiExemple[];
};

// One usage scenario: prop name → value to apply on a fresh instance. VARIANT
// values are exact option strings, BOOLEAN are booleans (or "true"/"false"
// strings, coerced), TEXT are the literal text content. INSTANCE_SWAP is
// ignored (left at the component default).
type AiExemple = {
  title?: string;
...
type AiDescriptionsStored = {
  ...
  generatedAt?: string;
  model?: string;
  // LLM-confirmed repeated sibling groups for the current target.
  repetitionGroups?: { parentKey: string; memberKeys: string[]; repKey: string }[];
  // LLM-chosen pin target per anchor key (Fix B).
  anchorTargets?: Record<string, string>;
...
figma.ui.onmessage = async (msg: {
  ...
  if (msg.type === "detect-repetition") {
    ...
    try {
      const { base, booleanPayload } = getAnatomyBaseAndOverrides(
        target,
        msg.anatomyVariant
      );
      if (!base) {
  ...
  if (msg.type === "detect-anchor-candidates") {
    ...
    try {
      type AnchorSection = {
        items: { anchorKey: string; candidates: AnchorCandidate[] }[];
        image:
          | {
              mediaType: string;
              base64: string;
      ...
      const out: { anatomy: AnchorSection; tokens: AnchorSection } = {
      ...
      const snapImage = async (
        ...
        try {
          ...
          if (bytes.byteLength > AI_IMAGE_MAX_BYTES) return null;
          return {
            mediaType: "image/png",
      ...
      if (an.base) {
        ...
        const inc = Array.isArray(msg.anatomyIncludedLayers)
          ? new Set<string>(msg.anatomyIncludedLayers)
          : null;
        if (inc) {
        ...
        if (out.anatomy.items.length > 0)
          out.anatomy.image = await snapImage(probe);
        probe.remove();
      }
      ...
      if (tk.base) {
        ...
        const incSet = Array.isArray(msg.tokenIncludedLayers)
          ? new Set<string>(msg.tokenIncludedLayers)
          : null;
        const keys: string[] = [];
        const seen = new Set<string>();
        const consider = (k: string): void => {
          if (k === "root" || seen.has(k)) return;
          if (rep.isRedundant(k)) return;
          if (incSet && !isAnchorInScope(k, incSet)) return;
          seen.add(k);
          keys.push(k);
        };
        ...
        if (out.tokens.items.length > 0)
          out.tokens.image = await snapImage(probe);
        probe.remove();
      }
  ...
  if (msg.type === "generate-doc") {
    ...
    if (ai) {
      ...
      if (ai.repetitionGroups) opts.repetitionGroups = ai.repetitionGroups;
      if (ai.anchorTargets) opts.anchorTargets = ai.anchorTargets;
      if (ai.exemples) opts.exemples = ai.exemples;
    }
  ...
  if (options.anatomy) {
    sheets.push(
      makeAdminSheet(
        target,
        "Anatomie",
        buildAnatomySection(target, options.anatomyVariant, options.anatomyIncludedLayers, options)
      )
    );
  }
  ...
  if (options.tokens) {
    sheets.push(
      makeAdminSheet(
        target,
        "Design tokens",
        await buildTokensSection(target, options.tokenVariant, options.tokenIncludedLayers, options)
      )
    );
  }
...
async function exportAsPdf(target: DocTarget, options: DocOptions): Promise<void> {
  ...
  if (options.tokens)
    pdfPages.push(
      ...(await buildPdfTokensPage(target, options.tokenVariant, options.tokenIncludedLayers, options))
    );
...
async function buildDocAsObject(target: DocTarget, options: DocOptions): Promise<DocData> {
  ...
  if (options.anatomy && base) {
    ...
    if (probedBase) {
      ...
      if (options.anatomyIncludedLayers !== undefined) {
        ...
        layers = findAllVisibleLayersWithPositions(probe).filter(
          (l) => inc.has(l.key) && !rep.isRedundant(l.key)
        );
  ...
  if (options.tokens && base) {
    ...
    if (tBase) {
      ...
      varUsages = varUsages.filter(
        (u) => u.anchorKey === "root" || !rep.isRedundant(u.anchorKey)
      );
      styleUsages = styleUsages.filter(
        (u) => u.anchorKey === "root" || !rep.isRedundant(u.anchorKey)
      );
...
async function buildTokensSectionForWidth(
  ...
  if (opts && opts.anchorTargets) {
    for (const ak of Object.keys(opts.anchorTargets)) {
      ...
      if (b) anchorTargetBoxes.set(ak, b);
    }
  }
  probe.remove();
  ...
  const tokenAnchorTarget = (u: {
    ...
    anchorTargetW: number;
    anchorTargetH: number;
  }) => {
    ...
    return b
      ? {
          ...
          targetW: b.w,
          targetH: b.h,
          targetKey: opts?.anchorTargets?.[u.anchorKey],
        }
      : {
...
function isVisiblePaintList(p: unknown): boolean {
  if (p === figma.mixed) return true;
  if (!Array.isArray(p)) return false;
  return (p as ReadonlyArray<Paint>).some(
    (paint) =>
      !!paint &&
      paint.visible !== false &&
      (typeof (paint as { opacity?: number }).opacity !== "number" ||
        (paint as { opacity: number }).opacity > 0)
  );
}

function nodeIsContent(n: SceneNode): boolean {
  if (n.visible === false) return false;
  if (n.type === "TEXT")
    return ((n as TextNode).characters || "").trim().length > 0;
  if (
    n.type === "INSTANCE" ||
    n.type === "COMPONENT" ||
...
function hasVisibleContent(n: SceneNode, budget: number): boolean {
  if (n.visible === false) return false;
  if (nodeIsContent(n)) return true;
  if (budget <= 0 || !("children" in n)) return false;
  for (const c of (n as ChildrenMixin & SceneNode).children) {
    if (hasVisibleContent(c, budget - 1)) return true;
  }
  return false;
}
...
function isLeafContentNode(n: SceneNode): boolean {
  if (n.visible === false) return false;
  if (n.type === "TEXT")
    return ((n as TextNode).characters || "").trim().length > 0;
  if (
  ...
  if ("children" in n) {
    for (const c of (n as ChildrenMixin & SceneNode).children) {
      if (hasVisibleContent(c, 2)) return false;
    }
  }
  return true;
}
...
function contentBoundsOf(
  ...
  const visit = (
    ...
    if (depth > 0 && "children" in n) {
      for (const c of (n as ChildrenMixin & SceneNode).children) {
        const clm = c as unknown as LayoutMixin;
...
function liveTargetRect(
  inst: InstanceNode,
  key: string
): { x: number; y: number; w: number; h: number } | null {
  if (!key || key === "root") return null;
  let cur: SceneNode = inst;
  let x = 0;
  let y = 0;
  for (const seg of key.split("/")) {
    ...
    const ch: SceneNode | undefined = (cur as ChildrenMixin & SceneNode)
      .children[idx];
    if (!ch) return null;
    const lm = ch as unknown as LayoutMixin;
    x += lm.x;
    y += lm.y;
    cur = ch;
  }
...
// Compact per-node descriptor for the LLM arbitration payload (geometry +
// what actually paints) — a lean "board export", no getCSSAsync fan-out.
...
function collectAnchorCandidates(
  ...
  if (!anchorKey || anchorKey === "root") return out;
  // Locate the documented node + its absolute offset.
  let cur: SceneNode = root;
  let baseX = 0;
  let baseY = 0;
  for (const seg of anchorKey.split("/")) {
    if (!("children" in cur)) return out;
    const idx = parseInt(seg, 10);
    if (!Number.isFinite(idx)) return out;
    const ch: SceneNode | undefined = (cur as ChildrenMixin & SceneNode)
      .children[idx];
    if (!ch) return out;
    const lm = ch as unknown as LayoutMixin;
  ...
  const recurse = (
    ...
    if (depth <= 0 || out.length >= 20 || !("children" in n)) return;
    const cs = (n as ChildrenMixin & SceneNode).children;
    for (let i = 0; i < cs.length; i++) {
      const c = cs[i];
      if (c.visible === false) continue;
      if (out.length >= 20) break;
      const lm = c as unknown as LayoutMixin;
      const cx = x + lm.x;
      const cy = y + lm.y;
      const ck = `${key}/${i}`;
      push(c, ck, cx, cy);
...
function makeLeaderDot(cx: number, cy: number): EllipseNode {
...
type PinAnchor = {
  ...
  targetW?: number;
  targetH?: number;
  // LLM-chosen absolute key (Fix B): when set, the post-rescale leader
  // retarget resolves THIS node's live box directly (no heuristic drill),
  // so B's pick survives Fix 3's re-derivation.
  targetKey?: string;
};
...
function buildPinnedVisualBlock(
  ...
  for (let i = 0; i < placements.length; i++) {
    ...
    const live = anchors[i].targetKey
      ? boxForKeyOnInstance(inst, anchors[i].targetKey as string)
...
function applyExempleProps(
  ...
  for (const k of Object.keys(scenario)) {
    ...
    if (info.type === "BOOLEAN") {
      ...
      if (opts.length === 0 || opts.indexOf(s) !== -1) payload[info.rawKey] = s;
...
async function buildExemplesSection(
  ...
  const props = extractProps(target);
  const byName = new Map<string, PropInfo>();
  const byRawKey = new Map<string, PropInfo>();
  ...
  for (let i = 0; i < placed.length; i++) {
    contentH += placed[i].h;
    if (i < placed.length - 1) contentH += EXEMPLE_GAP;
  }
  ...
  for (const p of placed) {
    ...
    if (p.scale < 1) p.inst.rescale(p.scale);
...
function findNamedLayersOnInstance(
  inst: InstanceNode,
...
// ─── Repeated-sibling detection (Breadcrumb items, RadioSet options…) ───────
//
// Components often repeat an identical child N times. Documenting each clone
// is noise, so we collapse a group of identical siblings to its first member
// ("representative") and treat the rest as redundant. Keys use the SAME
// child-index path scheme as AnatomyLayer.key and VarUsage.anchorKey
// (root child = "0", nested = "0/1"), so one detector serves both the
// Anatomie and Variables liées sections. The "root"/"" sentinels (the
// instance itself) have no siblings and are never redundant.
...
type RepetitionResult = {
  ...
  representativeOf: (key: string) => string | null;
};

const REPETITION_MIN_GROUP = 2;

function makeRepetitionResult(groups: RepetitionGroup[]): RepetitionResult {
  const redundantRoots: { root: string; repKey: string }[] = [];
  for (const g of groups) {
    for (const m of g.memberKeys) {
      if (m === g.repKey) continue;
      redundantRoots.push({ root: m, repKey: g.repKey });
    }
  }
  const coveredBy = (
    key: string
  ): { root: string; repKey: string } | null => {
    if (key === "" || key === "root") return null;
    for (const r of redundantRoots) {
      if (key === r.root || key.startsWith(r.root + "/")) return r;
    }
    return null;
  };
  return {
...
function detectRepeatedSiblingGroups(inst: InstanceNode): RepetitionResult {
  const groups: RepetitionGroup[] = [];
  const recurse = (node: SceneNode, depth: number, parentKey: string): void => {
    if (depth > ANATOMY_TREE_MAX_DEPTH) return;
    if (!("children" in node)) return;
    const container = node as ChildrenMixin & SceneNode;
    const buckets = new Map<string, { key: string; node: SceneNode }[]>();
    ...
    const redundantHere = new Set<string>();
    for (const sig of ordered) {
      const members = buckets.get(sig)!;
      if (members.length < REPETITION_MIN_GROUP) continue;
    ...
    for (let i = 0; i < container.children.length; i++) {
      ...
      if (child.visible === false) continue;
      const key = parentKey ? `${parentKey}/${i}` : String(i);
      if (redundantHere.has(key)) continue;
      recurse(child, depth + 1, key);
    }
  };
  recurse(inst, 0, "");
...
function resolveRepetition(
  ...
  if (confirmed !== undefined) {
    const groups: RepetitionGroup[] = confirmed
      .filter(
        (g) =>
          g &&
...
function buildKeyNodeIndex(inst: InstanceNode): Map<string, SceneNode> {
  const map = new Map<string, SceneNode>();
  const recurse = (node: SceneNode, depth: number, parentKey: string): void => {
    if (depth > ANATOMY_TREE_MAX_DEPTH) return;
    if (!("children" in node)) return;
    const c = node as ChildrenMixin & SceneNode;
...
function buildRepetitionPayload(
  inst: InstanceNode,
  groups: RepetitionGroup[],
  rootName: string
): RepetitionPayloadGroup[] {
  ...
  return groups.map((g) => {
    ...
    if (repNode && "children" in repNode) {
      const stack: SceneNode[] = [
        ...((repNode as ChildrenMixin & SceneNode).children as SceneNode[]),
      ];
      while (stack.length > 0 && sample.length < 8) {
        ...
        sample.push(n.name);
        if ("children" in n)
          stack.push(...((n as ChildrenMixin & SceneNode).children as SceneNode[]));
      }
    }
    return {
...
type VarUsage = {
  ...
  anchorW: number;
  anchorH: number;
  // Representative leaf box (resolveLeafTarget on the bound node) so the pin
  // aims at the real glyph/shape, not a wrapper. Falls back to the anchor box.
  anchorTargetX: number;
  anchorTargetY: number;
  anchorTargetW: number;
  anchorTargetH: number;
...
function previewTokensLayers(
  ...
  for (const u of collectVariableUsagesOnInstance(inst)) {
    if (u.anchorKey !== "root" && rep.isRedundant(u.anchorKey)) continue;
